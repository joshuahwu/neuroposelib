...
class DataStruct:
    ...
    @frame_id.setter
    def frame_id(self, frame_id: Union[List[int], np.array]):
    ...
    @exp_id.setter
    def exp_id(self, exp_id: Union[List[Union[str, int]], np.array]):
    ...
    def read_config(self, filepath, config_params: Optional[List[str]] = None):
        """
    ...
    def load_meta(
        ...
        meta_by_frame = (
            meta.iloc[self.exp_id].reset_index().rename(columns={"index": "exp_id"})
        )
    ...
    def load_feats(
        ...
        try:
            frames_with_good_tracking = (
                np.squeeze(
                    analysisstruct["frames_with_good_tracking"][0][0].astype(int)
                )
                - 1
            )
        except:
            frames_with_good_tracking = (
                np.squeeze(
                    analysisstruct["frames_with_good_tracking"][0][1].astype(int)
                )
                - 1
            )

        if self.exp_ids_full is None:
            exp_ids_full = np.squeeze(
                hdf5storage.loadmat(self.pose_path, variable_names=[self.exp_key])[
                    self.exp_key
                ].astype(int)
            )
    ...
    def load_pose(
        ...
        try:
            ...
            # if self.exp_ids_full is None:
            #     exp_ids_full = np.squeeze(h5py.File(self.pose_path)[self.exp_key])[self.exp_key].astype(int)
        except:
            print("Detected older version of '.mat' file")
            f = hdf5storage.loadmat(self.pose_path, variable_names=["predictions"])[
                "predictions"
            ]
            mat_v7 = False
    ...
    def load_connectivity(
        ...
        self.connectivity = Connectivity().load(
            skeleton_path=self.skeleton_path, skeleton_name=self.skeleton_name
        )
...
class Connectivity:
    ...
    def load(self, skeleton_path: str, skeleton_name: str = "mouse20"):
        """
        ...
        if skeleton_path.endswith(".py"):
            ...
            self.colors = con.COLOR_DICT[
                skeleton_name
            ]  # color to be plotted for each linkage
