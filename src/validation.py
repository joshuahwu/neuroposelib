...
class KFoldEmbed:
    ...
    def run(self,
        ...
        for param_val in param_range:
            ...
            self.plot_scatter(temp_embedding,
                              kf_embedding,
                              label = param_val,
                              out_path = self.out_path)
            mse_by_frame = np.sum((temp_embedding - kf_embedding)**2, axis=1) #squared error for each point
    ...
    def plot_error(self,
        ...
        for key in err_dict.keys():
            ...
            plt.savefig(''.join([out_path,'kfold_test/',self.param,'_',key,'.png']), dpi=400)
            plt.ylabel(str(key))
    ...
    def plot_scatter(self,
                     ...
                     out_path: Optional[str] = None):
        f = plt.figure()
        plt.scatter(temp_embedding[:,0], temp_embedding[:,1], marker='.', s=3, linewidths=0, alpha=0.5,
                    c='b', label='Targets')
        plt.scatter(kf_embedding[:,0], kf_embedding[:,1], marker='.', s=3, linewidths=0, alpha=0.5,
                    c='m', label='CV Predictions')
        plt.legend()
        plt.savefig(''.join([out_path,'kfold_test/embed_',self.param,'_',str(label),'.png']), dpi=400)
        plt.close()
