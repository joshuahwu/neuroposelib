...
def meta(path,
         id: Optional[List[Union[str, int]]]=None):
...
def features_mat(analysis_path: Optional[str]=None,
    ...
    analysisstruct = hdf5storage.loadmat(analysis_path, 
...
def pose(path: str,
    ...
    if exp_key:
        ids = np.squeeze(hdf5storage.loadmat(path, variable_names=[exp_key])[exp_key].astype(int))
...
def connectivity(path: str, 
    ...
    connectivity = Connectivity(joint_names = joint_names,
                                colors = colors,
                                links = links,
                                angles = angles)
