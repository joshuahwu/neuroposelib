...
def get_data(
    ...
    predictions_filepath: save_data_AVG.mat file path
    skeleton_path: file path for skeletons file
    com3d_filepath: com3d_used.mat filepath
    """
    if (
        dannceMat_filepath == None
        or predictions_filepath == None
        or skeleton_path == None
    ):
        print("One or more file paths is missing. Please provide all file paths.")
    elif (
        os.path.exists(dannceMat_filepath)
        and os.path.exists(predictions_filepath)
        and os.path.exists(skeleton_path)
    ):
        ...
        predictions = sio.loadmat(predictions_filepath)
...
def get_projected_points(
    ...
    for ncam in range(len(params)):
        ...
        imagePoints_blk = dops.project_to2d(
            tpred_bulk, camParam["IntrinsicMatrix"], rotation, translation
        )[:, :2]

        imagePoints_blk = dops.distortPoints(
            imagePoints_blk,
            camParam["IntrinsicMatrix"],
            np.squeeze(camParam["RadialDistortion"]),
            np.squeeze(camParam["TangentialDistortion"]),
        ).T
...
def plot_projected_points(
    ...
    with writer.saving(fig, video_save_path, dpi=300):
        ...
        for i in range(start_sample, start_sample + max_samples):
            ...
            fr = sync[0]["data_frame"][
                (np.where(sync[0]["data_sampleID"] == predictions["sampleID"][0][i]))[
                    0
                ].squeeze()
            ]
...
def driver(
    ...
    cam_names, sync, params, skeleton, predictions, com_3d = get_data(
        dannceMat_filepath=dannceMat_filepath,
        predictions_filepath=predictions_filepath,
        skeleton_path=skeleton_path,
    )

    cameraParams, rot, trans, mirror, links, goodmarks = get_camParams(
        params=params, skeleton=skeleton, exclude_joints=exclude_joints
    )

    imagePoints_agg, com_2d_agg = get_projected_points(
        predictions, params, cameraParams, rot, trans, mirror, com_3d
    )
