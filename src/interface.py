...
def run_analysis(params_config: str, ds: DataStruct):  # ,
    #  paths_config: str):
    ...
    if params["analysis"] == "exp_embed":
        ...
        ds_exp, embedder = embed_pipe(
            ds_exp, params, save_embedder=params["save_embedder"]
        )
...
def embed_pipe(dstruct: DataStruct, params: dict, save_embedder: bool = True):
    """ """
    ...
    if "batch_embed" not in params:  # Single embedding no batching
        ...
        if params["load_embedder"] is None:
            ...
            embedder = embedder.fit(
                data=dstruct.features,
        ...
        # import pdb; pdb.set_trace()
        embedder.embed(
            **embed_params, save_self=True
        )  # Embed template with chosen params

        dstruct.embed_vals = embedder.predict(
            dstruct.features
        )  # Embed all input points
...
def watershed_cluster(
    ...
    vis.scatter(
        dstruct.embed_vals,
        filepath="".join([dstruct.out_path, plots_label, "_scatter.png"]),
    )
